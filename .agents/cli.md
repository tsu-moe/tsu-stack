...
- Parse external data at the command boundary. Keep one-off schemas inline; extract a colocated `*.type.ts` contract only for reuse. Add parser helpers only for decoding, normalization, recovery, composition, or CLI-specific error mapping.
...
Each supported branch must continue to provide the root package manifest and any paths touched by its applicable transformations. Keep the application testing foundation available on every branch: package-owned API unit configuration and procedure tests, the web Playwright config and smoke spec, and the workspace test tasks that expose both lanes. CI generates every variant on Ubuntu and a representative variant on Windows and macOS. The representative project is installed from a fresh lockfile, checked, unit tested, browser tested, and built.
...
The `cloudflare-d1` hook owns Wrangler authentication, remote D1 creation, binding updates, and migrations. Remote resource creation occurs only when the user explicitly selects `--setup cloudflare`, after project files and dependencies are ready. Before creating anything, read `wrangler whoami --json` from `apps/web`, show the available account names and IDs, and require the user to select the owner. Persist the selected `account_id` in `apps/web/wrangler.jsonc`. The account prompt must offer a Wrangler logout/login flow and cancellation that preserves the generated project. Preflight remote D1 names and require explicit confirmation before reusing a matching database; when a name is already taken, also let the user supply another remote database name. Recheck supplied names against the preflight list so an existing database is never silently replaced or rebound. Keep the application binding name `DB` stable because it is part of the generated runtime contract. Treat bindings already written by Wrangler as success. Run remote migrations directly through app-local Wrangler and retry transient failures immediately after database creation. After successful remote provisioning and migration, apply the local D1 migrations using the finalized Wrangler binding so development works immediately. `--setup local` applies only the local D1 migrations without requiring Docker. If provisioning partially fails, preserve the generated project and report the account, database name/ID, underlying error, and app-local recovery command.

This guide is for maintainers of the generator. The transformation must remove `.agents/cli.md` from generated projects, along with `tools/create-tsu-stack`; the remaining `.agents` files, including testing and oRPC-testing guidance, continue to document application development. Generated projects must not inherit the package release workflow, generator smoke matrix, or root release script. Replace them with the application-only CI workflow that installs, prepares the local environment, checks, runs unit tests, installs Chromium, runs the browser smoke, and builds the generated workspace. Transformation fixtures must assert the canonical root test commands and browser installation step.
